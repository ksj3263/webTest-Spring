...
$(document).ready(function(){
    ...
    $('#property option').each(function() {
		if($(this).val() == "${skin.s_property}") {
			$(this).attr("selected", "selected");
		}
	});
      
	$('#tier option').each(function() {
		if($(this).val() == "${skin.s_tier}") {
			$(this).attr("selected", "selected");
		}
	});          
      
    let date = "${skin.s_date}".split("-");
    let year = date[0];
...
<form id="form" method="post" enctype="multipart/form-data">
	...
	<p> 스킨명 : <input type="text" name="s_name" value="${skin.s_name }"></p>
	<p> 선수명 : <input type="text" name="p_name" value="${skin.p_name }"> <button type="button" onclick="dup();">확인</button></p>
	<p> 이미지 : <input type="file" name="s_image" onchange="setThumbnail(event);"></p>	
	<div id="image_container">
		<img src="${skin.s_full }" width=236px height=338px>
	</div>
	<script>
	...
	<p> 일러스트레이터 : <input type="text" name="s_ill" value="${skin.s_ill }"></p>
	<p> CV : <input type="text" name="s_cv" value="${skin.s_cv }"></p>
	<input type="button" value="완료" onclick="transfer();" id="com"> 
</form>
<a href="/skin_detail?s_num=${skin.s_num }">돌아가기</a>
</body>
<script>
function dup() {
